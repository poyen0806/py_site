...
void main() {
  ...
  testWidgets('ThemeToggleButton widget renders correctly', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('ThemeToggleButton renders correct icon in light mode', (
    WidgetTester tester,
  ) async {
    when(() => mockThemeBloc.state).thenReturn(ThemeState(isDarkMode: false));
  ...
  testWidgets('ThemeToggleButton renders correct icon in dark mode', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('ThemeToggleButton toggles theme on press in light mode', (
    WidgetTester tester,
  ) async {
    when(() => mockThemeBloc.state).thenReturn(ThemeState(isDarkMode: false));
    ...
    verify(
      () => mockThemeBloc.add(SystemThemeChanged(Brightness.dark)),
    ).called(1);
  });

  testWidgets('ThemeToggleButton toggles theme on press in dark mode', (
    WidgetTester tester,
  ) async {
    when(() => mockThemeBloc.state).thenReturn(ThemeState(isDarkMode: true));
    ...
    verify(
      () => mockThemeBloc.add(SystemThemeChanged(Brightness.light)),
    ).called(1);
  });
}
